//============================================================================
// Name        : SpikingNeuronsReconstructer.cpp
...
int main() {
	...
	// counters for loops
	int n0, n1, t, n00, n11;	//some helper variables for loops

	// initialize 4 control parameter
	double a = 0.02;//affect recovery speed of u //higher value = fast --> and opposite
	double b = 0.2;			//affect u //how strong u and v are coupled together
	double c = -55;	//-65;	//value of membrane recovery variable u after action potential occured
	double d = 4;			//affect afterspike reset difference of u
	...
	//iniialize
	double I[MAX_NEURONS][MAX_TIME];//Input current which is given if a connected neuron releases an action potential one time step before
	double v[MAX_NEURONS][MAX_TIME];			//membrane potential
	double u[MAX_NEURONS][MAX_TIME];			//membrane recovery variable
	double weight[MAX_NEURONS][MAX_NEURONS];	//real weight weightMatrix

	// some test Variables
	double onlyInput[MAX_NEURONS][MAX_TIME];//random input current between 0 - 29
	double Vfactor[MAX_NEURONS][MAX_TIME];		//Eugene Itzikevich's --> v'
	double Spike[MAX_NEURONS][MAX_TIME];//help array for recognizing if a neuron spikes // values [0;1]
...
//	cout << "\n\n\n\n U\n\n\n\n\n\n\n\n V[mV]\n\n\n\n\n\n\n\n\n\n\n I[mV]";
	...
	// ---------------------- end of simulation ---------------------------------------
	cout << "Start reconstruction.\n";
	start = clock(); 		//for measuring duration of recontruction
	// ---------------------- here starts the reconstruction --------------------------
	for (double aa = 0.01; aa < 0.1; aa += 0.002) {

		for (double bb = 0; bb < 0.3; bb += 0.01) {
			...
			for (int neuron = 0; neuron < MAX_NEURONS; neuron++) {
				...
				for (t = 0; t < MAX_TIME; t++) {
					...
					// Uguess[o] = b * c
					if (t == 0) {
						Uguess[neuron][0] = u[neuron][0];// parameter to guess set to original
					...
					Uguess[neuron][t] = Uguess[neuron][t - 1]
							+ 0.5 * a
									* (b * v[neuron][t - 1]
											- Uguess[neuron][t - 1]);
					...
					if (v[neuron][t - 1] == 30)
						Uguess[neuron][t] = Uguess[neuron][t - 1] + d;
			...
			for (int neuron = 0; neuron < MAX_NEURONS; neuron++) {
				...
				for (n1 = 0; n1 < MAX_NEURONS; n1++) {
					fprintf(weightMatrixFile,
							...
							weight[neuron][n1], calculatedWeightSolution[n1]);
					Wguess[neuron][n1] = calculatedWeightSolution[n1];
				}
